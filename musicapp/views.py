...
CLIENT_ID = "ea00ce9084b1427aa034450191562888"
CLIENT_SECRET = "4cc3d7be06124f4496e713ca06504242"
...
def recommend(song):
    ...
    for i in distances[1:6]:
        ...
        single_recommended = {'key':i,
                              'recommended_music_poster':get_song_album_cover_url(music.iloc[i[0]].song, artist),
                              'recommended_music_name':music.iloc[i[0]].song,
                              'artist':artist,
                              }
        recommended.append(single_recommended)

    return recommended


def generate_recommendations(request):
    # Your logic for generating recommendations goes here
    # For now, let's just return a simple response
    music_list = music['song'].values
    if len(request.GET)>0:
        search_query = request.GET.get('song')
        recommended = recommend(search_query)
        context={
        'music_list':music_list,
        'is_searched':True,
        'recommended':recommended,
        }
        return render(request, 'musicapp/generate_recommendations.html',context=context)
